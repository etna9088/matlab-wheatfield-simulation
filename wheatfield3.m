...
death = 0;
results = zeros(1,number_of_expts);

%-------------------- OUTER SIMULATION LOOP --------------------
for expt = 1:number_of_expts
 
    P(1) = 1000; %initial condition for each experiment
    
    %-------------------- INNER SIMULATION LOOP ----------------- 
    for t = 1:10
        if (rand <= chance_of_sun)
            P(t+1) = P(t) + (birth_sunny-death)*P(t);
            P(t+1) = P(t+1) - lost_to_Puccinia;
        else
            P(t+1) = P(t) + (birth_cloudy-death)*P(t);
            P(t+1) = P(t+1) - lost_to_Puccinia;
        end
